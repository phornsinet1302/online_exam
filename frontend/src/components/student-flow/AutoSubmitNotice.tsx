...
export function AutoSubmitNotice() {
  ...
  // ?reason=violation is set when an anti-cheating rule submitted the exam.
  const violation = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("reason") === "violation";
  const Icon = violation ? ShieldAlert : Clock;
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{background:CREAM}}>
      <div className="bg-white rounded-3xl border border-gray-100 shadow-xl p-10 max-w-md w-full text-center overflow-hidden relative">
        ...
        <h1 className="text-2xl font-black mb-2" style={{fontFamily:U,color:INK}}>{violation ? "Exam submitted" : "Time's Up!"}</h1>
        <p className="text-sm text-gray-500 mb-4" style={{fontFamily:I}}>
          {violation
            ? "Your exam was automatically submitted because of activity that isn't allowed during this exam. Your teacher has been notified."
            : "Your exam has been automatically submitted because the time limit was reached."}
        </p>
